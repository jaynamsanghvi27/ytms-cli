...
export class TrainerCalendarComponent {
  ...
  fetchAllEvents() {
    //Reset Events TO BLANK BEFORE FETCHING EVENTS FROM DATABASE
      ...
      formdata.append('username',this.jwtservice.getUserNameFromToken(this.authService.getToken()));
      formdata.append('date',this.datepipe.transform(this.viewDate,'dd-MMM-yyyy')!);
      this.calendarService.getAllEventsForUser(formdata).subscribe(
        (response: any) => {
  ...
  //THIS METHOD IS USED TO FETCH ALL THE TRAINERS IN THE ORGANIZATION
  fetchAllTrainers() {
